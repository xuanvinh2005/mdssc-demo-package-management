                   ...
                   const Aws::String &region = "")
{
    ...
    if (!file_exists(file_name)) {
        std::cout << "ERROR: NoSuchFile: The specified file does not exist" << std::endl;
        return false;
    }
    ...
    ws::Auth::AWSCredentials credentials;
    credentials.SetAWSAccessKeyId(Aws::String("AKIAPRT746DSCQ35LPTZ"));
    credentials.SetAWSSecretKey(Aws::String("AWK0oa39TuwUUEsnbT1uuYQfeYh/sn7xpAbC5TMO"));
    ...
    const std::shared_ptr<Aws::IOStream> input_data
        = Aws::MakeShared<Aws::FStream>("SampleAllocationTag", file_name.c_str(), std::ios_base::in | std::ios_base::binary);
...
std::vector<Credentials> plsDontCommit {
    { "AKIATESTCREDENTIAL12", "oRRVk+GduA5MdedUxP7X0N9VHnAUsmNmcHloIl5Q" },
    { "AKIAZZZZZZZZZZZZZZZZ", "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" },
    { "AKIA87DSTM420PXCEEIK", "Wb0maFhDjoTx3NO1966WmUSy6lEaTarBAMP/H0Pv" },
};
...
int main(int argc, char **argv)
{
    ...
    Aws::InitAPI(options);
    {
        ...
        if (put_s3_object(bucket_name, object_name, file_name, region)) {
            std::cout << "Put file " << file_name << " to S3 bucket " << bucket_name << " as object " << object_name
                      << std::endl;
        }
    }
